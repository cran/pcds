...
Or, alternatively, we can use the `plotDeltri` function in `pcds` to obtain the same plot by executing `plotDeltri(Xp,Yp,xlab="x",ylab="y",main="X points and Delaunay Triangulation of Y Points")` command.

The number of Delaunay triangles based on $\Y$ points can be obtained by the function `num.del.tri`.
```{r eval=F}
num.del.tri(Yp)
#> [1] 4
```
...
Number of arcs of the AS-PCD can be computed by the function `NumArcsAS`,
...
- `M`, the center of the triangle used to construct the vertex regions. `"CC"` stands for circumcenter of each Delaunay triangle `tri` or 3D point in barycentric coordinates which serves as a center in the interior of each Delaunay triangle;
...
* `num.in.conhull`: Number of $\X$ points in the convex hull of $\Y$ points
...
The incidence matrix of the AS-PCD can be found by `IncMatAS`. 
...
The function `dom.greedy` finds the approximate domination number (an upper bound) and also provides the indices of the points in the approximate dominating set.
On the other hand, the function `dom.exact` finds the exact domination number and the indices of the points
in an exact dominating set. 
`dom.exact` might take a long time for large  $n_x$ (e.g. $n_x \ge 19$), 
...
- `Xp`, `Yp` and `M` are same as in the function `NumArcsAS`,
...
has the same arguments as in `NumArcsAS`.
...
Number of arcs of the PE-PCD can be computed by the function `NumArcsPE`.
The function returns the same type of output as the function `NumArcsAS`.
...
The incidence matrix of the PE-PCD can be found by `IncMatPE`. 
...
based on the arc density of PE-PCDs using the function `TSArcDensPE`
which takes the arguments
...
We first provide two functions to compute the domination number of PE-PCDs: `PEdom` and `PEdom.nd`.

The function `PEdom` takes the same arguments as `NumArcsPE` 
...
On the other hand, `PEdom.nd` takes only the arguments `Xp,Yp,r` 
and returns the same output as in `PEdom` function, 
...
These two functions functions are different from the function `dom.greedy` since they give an exact minimum dominating set and 
...
`TSDomPEBin` or `TSDomPENorm`.
Each of these functions is an object of class "`htest`" (i.e., hypothesis test) 
and performs the same hypothesis test as in `TSArcDensPE`.
...
and other variables are as in `TSArcDensPE`.
...
Here, the PE proximity region is constructed with the expansion parameter $r \in (1,1.5]$ and $M$-vertex regions where `M`
is a center that yields non-degenerate asymptotic distribution of the domination number.

The test statistic in `TSDomPEBin` is based on the binomial distribution, 
...
The test statistic in `TSDomPENorm` is based on the normal approximation to the binomial distribution.
...
For example, `TSArcDensPE(Xp,Yp,r,ch=TRUE)` would yield the convex hull corrected version 
of the arc-based test of spatial interaction.

## Summary and Visualization with Central Similarity PCDs

The functions for CS-PCD have similar arguments as the `NumArcsPE` with the expansion parameter $t >0$.
...
Number of arcs of the CS-PCD can be computed by the function `NumArcsCS`,
which takes similar arguments and returns the similar output items as in `NumArcsPE`.
...
The incidence matrix of the CS-PCD can be found by `IncMatCS`. 
Below, we only print the top 6 rows of the incidence matrix.
...
We can test the spatial interaction between two classes or species based on the arc density of CS-PCDs using the function `TSArcDensCS`.
...
Moreover, it takes similar arguments and returns similar output as in `TSArcDensPE` function, 
...
For example, `TSArcDensCS(Xp,Yp,tau,ch=TRUE)` would yield the convex hull corrected version of 
