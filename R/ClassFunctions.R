...
plot.Lines<-function (x, asp=NA, xlab="x",ylab="y",...)
{
  ...
  } else
  {
    plot(xp,yp,asp=asp,type="l",xlab=xlab,ylab=ylab,
         xlim=Xlim+xd*c(-.05,.05),ylim=Ylim+yd*c(-.05,.05),main=x$mtitle, ...)
    text(rbind(c(median(xp),x$intercept+Sl*median(xp)))+pf*3, col=1,eqn.lab)
  }
  points(pts)
...
plot.TriLines<-function (x,xlab="x",ylab="y", ...)
{
  ...
  xfence<-abs(A[1]-B[1])*.25 #how far to go at the lower and upper ends in the x-coordinate
  xp<-seq(min(A[1],B[1])-xfence,max(A[1],B[1])+xfence,l=3) #try also l=10, 20 or 100
...
plot.Lines3D<-function (x,xlab="x",ylab="y",zlab="z",phi = 40, theta = 40,...)
{
  ...
  if (x$vec.names[2]=="normal vector")
  {
    ...
    lines3D(xc, yc, zc, bty = "g",pch = 20, cex = 2, ticktype = "detailed",add=TRUE)
    if (!is.null(Pts))
    ...
    text3D(Dr[1]+Bv[1]/2,Dr[2]+Bv[2]/2,Dr[3]+zf+Bv[3]/2,labels=x$vec.names[2],add=TRUE)
    arrows3D(Dr[1],Dr[2],Dr[3]+zf,Dr[1]+Bv[1],Dr[2]+Bv[2],Dr[3]+zf+Bv[3], add=TRUE)
    text3D(A[1],A[2],A[3]+zf/2,labels=x$vec.names[1],add=TRUE)
    arrows3D(Dr[1]+Bv[1]/2,Dr[2]+Bv[2]/2,Dr[3]+3*zf+Bv[3]/2,Dr[1]+Bv[1]/2,Dr[2]+Bv[2]/2,Dr[3]+zf+Bv[3]/2,lty=2, add=TRUE)
  ...
  } else
  {
    ...
    arrows3D(A[1],A[2],A[3]-zf,A[1],A[2],A[3],lty=2, add=TRUE)
    text3D(A[1],A[2],A[3]-zf,labels="initial point",add=TRUE)
    text3D(Dr[1]+Bv[1]/2,Dr[2]+Bv[2]/2,Dr[3]+3*zf+Bv[3]/2,labels="direction vector",add=TRUE)
    arrows3D(Dr[1],Dr[2],Dr[3]+zf,Dr[1]+Bv[1],Dr[2]+Bv[2],Dr[3]+zf+Bv[3], add=TRUE)
    text3D(A[1],A[2],A[3]+zf/2,labels=x$vec.names[1],add=TRUE)
...
#' @param x.grid.size,y.grid.size the size of the grids for the \eqn{x} and \eqn{y} axes, default is 10 for both
#' @param theta,phi The angles defining the viewing direction, default is 40 for both.
...
plot.Planes<-function (x, x.grid.size=10, y.grid.size=10,xlab="x",ylab="y",zlab="z",phi = 40, theta = 40,...)
{
  ...
  persp3D(z = zgrid, x = xp, y = yp, xlab=xlab,ylab=ylab,zlab=zlab,phi=phi,theta=theta,
          xlim=xlim+xd*c(-.05,.05),ylim=ylim+yd*c(-.05,.05), zlim=zlim+zd*c(-.05,.05),
          main=x$main.title, ...) #plane spanned by points A, B, C
...
print.summary.Uniform <- function(x, ...)
{
  ...
  if (Dimn==2)
  {
    ...
    {
     # cat("\n Support of the Uniform Points is the convex hull of Y points  of which first 6 or fewer are printed \n")
     # print(x$yvec)

      cat("\n The x and y coordinates of the nodes on the boundary of the convex hull of Y points and their indices (labeled $i) \n")
      Yp<-x$Support
...
plot.Extrema<-function (x,asp=NA,xlab="",ylab="",zlab="", ...)
{
  ...
  if (dimn==1)
  {
    ...
    text(cbind(c(Y,Cent,x$region.centers)-.02*xd,-0.05),c("a","b",x$cent.name,x$region.names))
  } else if (dimn==2)
  {
    ...
    } else
    {
      ...
      txt.str<-c("A","B","C",x$cent.name,x$region.names)
    }
    text(xc,yc,txt.str)
...
#' and the heads (or end points) of the arcs of the PCD, parameter of the PCD, and various quantities (number of vertices,
