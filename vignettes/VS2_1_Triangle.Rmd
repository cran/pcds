...
It takes the arguments `Xp,tri,M="CC",in.tri.only` where `Xp` is the data set (or $\X$ points)
...
if `in.tri.only=TRUE`, arc density is computed only for 
...
It takes the arguments `Xp,tri,r,M,in.tri.only` where `Xp` is the data set (or $\X$ points)
`r,tri,M` are as above, `in.tri.only` is as in `ASarc.dens.tri`,
...
<!-- Arc density can be adjusted (or corrected) for the points outside the triangle by using the option `in.tri.only=TRUE` as in the `ASarc.dens.tri`. -->
...
It takes the arguments `Xp,tri,t,M=c(1,1,1),in.tri.only=FALSE` where `Xp` is the data set (or $\X$ points)
`tri,t,M=c(1,1,1),in.tri.only=FALSE` are as above
and `in.tri.only` is as in `ASarc.dens.tri`.
<!-- Arc density can be adjusted (or corrected) for the points outside the triangle by using the option `in.tri.only=TRUE` as in the `num.arcsPEtri`. -->
```{r eval=F }
