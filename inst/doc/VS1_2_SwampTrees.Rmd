...
Or, alternatively, 
we can use the `plotDeltri` function in `pcds` to obtain the plot of the Delaunay triangulation by executing the line: `plotDeltri(Xp,Yp,xlab="x",ylab="y",main="Live Tree Locations (solid squares) and Delaunay Triangulation of Dead Tree Locations")`.

The number of Delaunay triangles based on dead trees is obtained by the function `num.del.tri`.
```{r eval=F}
num.del.tri(Yp)
[1] 194
```
...
Number of arcs of the AS-PCD can be computed by the function `NumArcsAS` whose arguments and output is described in 
...
We can test the spatial pattern or interaction of segregation/association based on arc density of PE-PCD using the function `TSArcDensPE`
...
`TSDomPEBin` and `TSDomPENorm`.
We first provide two functions `PEdom` and `PEdom.nd` to compute the domination number of PE-PCDs 
(see Section "VS1_1_2DArtiData", for details):
...
domination number of the PE-PCDs with the function `TSDomPEBin`.
...
domination number of the PE-PCDs with the function `TSDomPENorm`.
...
We can test the spatial pattern or interaction of segregation/association based on arc density of CS-PCD using the function `TSArcDensCS`
(see Section "VS1_1_2DArtiData", for details):
