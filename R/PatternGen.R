...
#' @param Yp A set of 2D points representing the reference points. The generated points are segregated
#' (in a circular or radial fashion) from these points.
#' @param e A positive real number representing the radius of the balls centered at \code{Yp} points. These balls
...
#'
#' @seealso \code{\link{rasc.circ}}, \code{\link{rseg.std.tri}}, \code{\link{rsegII.std.tri}}, and \code{\link{rsegMT}}
#'
#' @author Elvan Ceyhan
#'
#' @examples
...
#'
#' @export rseg.circ
rseg.circ <- function(n,Yp,e,a1=min(Yp[,1]),a2=max(Yp[,1]),b1=min(Yp[,2]),b2=max(Yp[,2]))
{
  if (is.point(Yp))
  { Yp<-matrix(Yp,ncol=2)
  } else
  ...
  res<-list(
    ...
    ref.points=Yp, #reference points, i.e., points from which generated points are segregated
    tri.Yp=FALSE,
...
#' \eqn{n_y} being number of \code{Yp} points for various values of \code{e} under the association pattern
#' and \eqn{B(y_i,e)} is the ball centered at \eqn{y_i} with radius \code{e}.
#'
#' \code{e} must be positive and very large values of \code{e} provide patterns close to CSR.
#' \code{a1} is defaulted to the minimum of the \eqn{x}-coordinates of the \code{Yp} points,
#' \code{a2} is defaulted to the maximum of the \eqn{x}-coordinates of the \code{Yp} points,
#' \code{b1} is defaulted to the minimum of the \eqn{y}-coordinates of the \code{Yp} points,
#' \code{b2} is defaulted to the maximum of the \eqn{y}-coordinates of the \code{Yp} points.
#' This function is also very similar to \code{\link{rasc.matern}}, where \code{rasc.circ} needs the study window to be specified,
#' while \code{\link{rasc.matern}} does not.
#'
#' @param n A positive integer representing the number of points to be generated.
#' @param Yp A set of 2D points representing the reference points. The generated points are associated
#' (in a circular or radial fashion) with these points.
#' @param e A positive real number representing the radius of the balls centered at \code{Yp} points. Only these
...
#'
#' @seealso \code{\link{rseg.circ}}, \code{\link{rasc.std.tri}}, \code{\link{rascII.std.tri}},
...
#' yd<-Ylim[2]-Ylim[1]
#'
#' plot(Y,asp=1,xlab="x",ylab="y",main="Circular Association of X points with Y Points",
...
#' #try also Y<-cbind(runif(ny,a1,a2/2),runif(ny,b1,b2/2))
#'
#' Xdt<-rasc.circ(nx,Y,e,a1,a2,b1,b2)$gen.points
#' Xlim<-range(Xdt[,1],Y[,1]);
#' Ylim<-range(Xdt[,2],Y[,2])
#' xd<-Xlim[2]-Xlim[1]
#' yd<-Ylim[2]-Ylim[1]
...
#'
#' @export rasc.circ
rasc.circ <- function(n,Yp,e,a1=min(Yp[,1]),a2=max(Yp[,1]),b1=min(Yp[,2]),b2=max(Yp[,2]))
{
  if (is.point(Yp))
  { Yp<-matrix(Yp,ncol=2)
  } else
...
#' @description
#' An object of class \code{"Patterns"}.
#' Generates \code{n} 2D points uniformly in \eqn{\cup B(y_i,e)} where \eqn{Y_p=(y_1,y_2,\ldots,y_{n_y})}
#' with \eqn{n_y} being number of \code{Yp} points for various values of \code{e} under the association pattern
#' and \eqn{B(y_i,e)} is the ball centered at \eqn{y_i} with radius \code{e}.
#'
...
#' window \code{win}.
#'
#' The main difference of \code{rasc.matern} and \code{\link[spatstat.random]{rMatClust}} is that the parent points are \code{Yp} points
#' which are given beforehand and we do not discard them in the end in \code{rasc.matern} and the offspring points
#' are the points associated with the reference points, \code{Yp};
#' \code{e} must be positive and very large values of \code{e} provide patterns close to CSR.
#'
#' This function is also very similar to \code{\link{rasc.circ}}, where \code{\link{rasc.circ}} needs the study window to be specified,
#' while \code{rasc.matern} does not.
#'
#' @param n A positive integer representing the number of points to be generated.
#' @param Yp A set of 2D points representing the reference points. The generated points are associated
#' (in a Matern-cluster like fashion) with these points.
#' @param e A positive real number representing the radius of the balls centered at \code{Yp} points. Only
...
#' \item{xlimit,ylimit}{The possible ranges of the \eqn{x}- and \eqn{y}-coordinates of the generated points.}
#'
#' @seealso \code{\link{rasc.circ}}, \code{\link{rasc.std.tri}}, \code{\link{rascII.std.tri}},
#' \code{\link{rascMT}}, \code{\link{rseg.circ}}, and \code{\link[spatstat.random]{rMatClust}}
...
#' yd<-Ylim[2]-Ylim[1]
#'
#' plot(Y,asp=1,xlab="x",ylab="y",main="Matern-like Association of X points with Y Points",
#'      xlim=Xlim+xd*c(-.01,.01),ylim=Ylim+yd*c(-.01,.01),pch=16,col=2,lwd=2)
#' points(Xdt)
#'
#' a1<-0; a2<-10;
...
#' Generates \code{n} points uniformly in the first 1/6th of the standard equilateral triangle \eqn{T_e=(A,B,C)}
...
#'
#' @param n a positive integer representing number of uniform points to be generated
...
#'
#' @seealso \code{\link{runif.std.tri}}, \code{\link{runif.bas.tri}}, \code{\link{runif.tri}}, and \code{\link{runifMT}}
#'
#' @author Elvan Ceyhan
#'
#' @examples
...
#' @description
#' An object of class \code{"Uniform"}.
#' Generates \code{n} points uniformly in the standard equilateral triangle \eqn{T_e=T(A,B,C)}
#' with vertices \eqn{A=(0,0)}, \eqn{B=(1,0)}, and \eqn{C=(1/2,\sqrt{3}/2)}.
#'
#' @param n A positive integer representing the number of uniform points to be generated in the
...
#'
#' @seealso \code{\link{runif.bas.tri}}, \code{\link{runif.tri}}, and \code{\link{runifMT}}
#'
#' @author Elvan Ceyhan
#'
#' @examples
...
runif.std.tri <- function(n)
{
  ...
  res<-list(
    type=typ,
    mtitle=main.txt,
    tess.points=Te, #tessellation points whose convex hull constitutes the support of the uniform points
...
#' @description
#' An object of class \code{"Uniform"}.
#' Generates \code{n} points uniformly in the standard basic triangle \eqn{T_b=T((0,0),(1,0),(c_1,c_2))}
...
#' Hence standard basic triangle is useful for simulation studies under the uniformity hypothesis.
#'
#' @param n A positive integer representing the number of uniform points to be generated in the standard basic triangle.
#' @param c1,c2 Positive real numbers representing the top vertex in standard basic triangle
...
#' \item{type}{The type of the pattern from which points are to be generated}
#' \item{mtitle}{The \code{"main"} title for the plot of the point pattern}
#' \item{tess.points}{The vertices of the support of the uniformly generated points, it is the standard basic triangle
#' \eqn{T_b} for this function}
#' \item{gen.points}{The output set of generated points uniformly in the standard basic triangle}
#' \item{out.region}{The outer region which contains the support region, \code{NULL} for this function.}
#' \item{desc.pat}{Description of the point pattern from which points are to be generated}
#' \item{num.points}{The \code{vector} of two numbers, which are the number of generated points and the number
#' of vertices of the support points (here it is 3).}
...
#'
#' @seealso \code{\link{runif.std.tri}}, \code{\link{runif.tri}}, and \code{\link{runifMT}}
#'
#' @references
#' \insertAllCited{}
#'
...
#'
#' set.seed(1)
#' runif.bas.tri(1,c1,c2)
#' Xdt<-runif.bas.tri(n,c1,c2)
#' Xdt
#' summary(Xdt)
#' plot(Xdt)
#'
...
#' }
#'
#' @export runif.bas.tri
runif.bas.tri <- function(n,c1,c2)
{
  ...
  res<-list(
    type=typ,
    gen.points=X, #uniformly generated points in the standard basic triangle, Tb
    tess.points=Tb, #tessellation points whose convex hull constitutes the support of the uniform points
    desc.pat=txt, #description of the pattern
    mtitle=main.txt,
...
#'
#' @param n A positive integer representing the number of uniform points to be generated in the triangle.
...
#'
#' @seealso \code{\link{runif.std.tri}}, \code{\link{runif.bas.tri}}, and \code{\link{runifMT}}
#'
#' @author Elvan Ceyhan
#'
#' @examples
...
runif.tri <- function(n,tri)
{
  ...
  res<-list(
    ...
    tess.points=tri, #tessellation points whose convex hull constitutes the support of the uniform points
    desc.pat=txt, #description of the pattern
...
#'
#' @seealso \code{\link{rseg.circ}}, \code{\link{rasc.circ}},  \code{\link{rsegII.std.tri}}, and \code{\link{rsegMT}}
#'
#' @references
#' \insertAllCited{}
#'
...
#' @description
#' An object of class \code{"Patterns"}.
#' Generates \code{n} points uniformly in the support for Type I segregation in
...
#' standard equilateral triangle \eqn{T_e} as \eqn{delta=4 eps ^2/3} (see \code{rseg.std.tri}  function).
#'
...
#'
#' @param n A positive integer representing the number of points to be generated from the segregation pattern
...
#'
#' @seealso \code{\link{rasc.tri}}, \code{\link{rseg.std.tri}}, \code{\link{rsegII.std.tri}}, and \code{\link{rsegMT}}
#'
#' @references
#' \insertAllCited{}
#'
...
#' yd<-Ylim[2]-Ylim[1]
#'
#' plot(Tr,pch=".",xlab="",ylab="",main="Points from Type I Segregation \n in one Triangle",
#' xlim=Xlim+xd*c(-.05,.05),ylim=Ylim+yd*c(-.05,.05))
#' polygon(Tr)
#' points(Xp)
#' xc<-Tr[,1]+c(-.02,.02,.02)
...
rseg.tri <- function(n,tri,delta)
{
  ...
  res<-list(
    type=typ,
    parameters=param,
    gen.points=Xp, #generated points segregated from Y points (vertices of the triangle, tri)
...
#' vertices of \eqn{T_e} here}
#'
#' @seealso \code{\link{rseg.circ}}, \code{\link{rasc.circ}},  \code{\link{rseg.std.tri}}, and \code{\link{rsegMT}}
#'
#' @author Elvan Ceyhan
#'
#' @examples
...
#' @description
#' An object of class \code{"Patterns"}.
#' Generates \code{n} points uniformly in the support for Type I association in
#' a given triangle, \code{tri}.
...
#' \eqn{T_e} as \eqn{delta=4eps^2/3} (see \code{rseg.std.tri}  function).
#'
#' See (\insertCite{ceyhan:arc-density-PE,ceyhan:arc-density-CS,ceyhan:dom-num-NPE-Spat2011;textual}{pcds}) for more on
#' the association pattern.
#'
#' @param n A positive integer representing the number of points to be generated from the association pattern
#' in the triangle, \code{tri}.
#' @param tri A \eqn{3 \times 2} matrix with each row representing a vertex of the triangle.
...
#' \code{Yp} points}
#'
#' @seealso \code{\link{rseg.tri}}, \code{\link{rasc.std.tri}}, \code{\link{rascII.std.tri}}, and \code{\link{rascMT}}
#'
#' @references
#' \insertAllCited{}
#'
...
#' yd<-Ylim[2]-Ylim[1]
#'
#' plot(Tr,pch=".",xlab="",ylab="",main="Points from Type I Association \n in one Triangle",
#' xlim=Xlim+xd*c(-.05,.05),ylim=Ylim+yd*c(-.05,.05))
#' polygon(Tr)
#' points(Xp)
#' xc<-Tr[,1]+c(-.02,.02,.02)
...
#' }
#'
#' @export rascII.std.tri
rascII.std.tri <- function(n,eps)
{
  if (!is.point(eps,1) || eps<=0 || eps>=sqrt(3)/6 )
  {stop('eps must be a scalar in (0,sqrt(3)/6=0.2886751)')}
...
#'
#' @seealso \code{\link{runif.tri}}, \code{\link{runif.std.tri}}, and \code{\link{runif.bas.tri}},
#'
#' @references
#' \insertAllCited{}
#'
...
#' Xp<-runifMT(nx,Yp,DTY)$g #data under CSR in the convex hull of Ypoints
#'
#' Xlim<-range(Yp[,1])
#' Ylim<-range(Yp[,2])
#' xd<-Xlim[2]-Xlim[1]
...
#' plot(Xp, xlab=" ", ylab=" ",main="Uniform Points in Convex Hull of Y Points",
...
runifMT <- function(n,Yp,DTmesh=NULL)
{
  ...
  npts<-c(n,ny)
  names(npts)<-c("nx","ny")
  txt4pnts<-paste(names(npts)[1], " : the number of Uniform points\n",names(npts)[2], " : the number of points whose convex hull determines the support")

  res<-list(
    type=typ,
    mtitle=main.txt,
    gen.points=Xp, #uniformly generated points in the triangle, tri
...
#'
#' @seealso \code{\link{rseg.std.tri}} and \code{\link{rsegMT}}
#'
#' @author Elvan Ceyhan
#'
#' @examples
...
#' \code{Yp} points}
#'
#' @seealso \code{\link{rseg.circ}}, \code{\link{rseg.std.tri}}, \code{\link{rsegII.std.tri}}, and \code{\link{rascMT}}
#'
#' @references
#' \insertAllCited{}
#'
...
#' Xp<-rsegMT(nx,Yp,del,DTY,TRY)$gen.points #data under CSR in the convex hull of Ypoints
#'
#' Xlim<-range(Yp[,1])
#' Ylim<-range(Yp[,2])
#' xd<-Xlim[2]-Xlim[1]
...
#' xlab=" ", ylab=" ",xlim=Xlim+xd*c(-.05,.05),ylim=Ylim+yd*c(-.05,.05),type="n")
...
rsegMT <- function(n,Yp,delta,DTmesh=NULL,DTr=NULL)
{
  ...
  } else
  {
    ...
    res<-list(
      type=typ,
      parameters=param,
      gen.points=Xp, #generated points segregated from Y points (vertices of Y triangles)
...
#' \code{Yp} points}
#'
#' @seealso \code{\link{rasc.circ}}, \code{\link{rasc.std.tri}}, \code{\link{rascII.std.tri}}, and \code{\link{rsegMT}}
#'
#' @references
#' \insertAllCited{}
#'
...
#' DTY<-interp::tri.mesh(Yp[,1],Yp[,2],duplicate="remove")  #Delaunay triangulation based on Y points
#' TRY<-interp::triangles(DTY)[,1:3];
#' Xp<-rascMT(nx,Yp,del,DTY,TRY)$g #data under CSR in the convex hull of Ypoints
#'
#' Xlim<-range(Yp[,1])
#' Ylim<-range(Yp[,2])
#' xd<-Xlim[2]-Xlim[1]
...
rascMT <- function(n,Yp,delta,DTmesh=NULL,DTr=NULL)
{
  ...
  } else
  {
    ...
    res<-list(
      type=typ,
      parameters=param,
      gen.points=Xp, #generated points associated Y points (vertices of Y triangles)
...
#' s3d<-scatterplot3d(Xp, highlight.3d=TRUE,xlab="x",ylab="y",zlab="z",
...
runif.std.tetra <- function(n)
{
  ...
  res<-list(
    type=typ,
    mtitle=main.txt,
    tess.points=Th, #tessellation points whose convex hull constitutes the support of the uniform points
...
#' @description
#' An object of class \code{"Uniform"}.
#' Generates \code{n} points uniformly in the general tetrahedron \code{th} whose vertices are stacked row-wise.
#'
#' @param n A positive integer representing the number of uniform points to be generated in the tetrahedron.
...
#' zd<-Zlim[2]-Zlim[1]
#'
#' plot3D::scatter3D(Xp[,1],Xp[,2],Xp[,3], theta =225, phi = 30, bty = "g",
