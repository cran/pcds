...
(which is the Delaunay tessellation of $\Y$ points).
More specifically,
the proximity regions are defined with respect to the
Delaunay cells (i.e., intervals) based on order statistics of the $\Y$ points and 
vertex regions in each interval are based on the center $M_c=a+c\,(b-a)$ for the interval $[a,b]$ where $c \in (0,1)$.
Notice that Delaunay tessellation of $\Y$ points
...
For an interval, `int`$=(a,b)$, the parameterized center is $M_c=a+c(b-a)$.
...
based on `Yp` points,
* `int.num.arcs`: The `vector` of the number of arcs of the component of the PE-PCD in the
partition intervals (including the end intervals) based on `Yp` points,
* `part.int`: A list of partition intervals based on `Yp` points,
...
where `Jit` equals to the range of `Xp` and `Yp` multiplied by `Jit`).
...
- `centers`, a logical argument, if `TRUE`, plot includes the centers of the intervals
...
### Testing Spatial Interaction and Uniformity with the PE-PCDs {#sec:testing-1D-arti-data-PE-PCD} 
We can test the spatial interaction between two classes/species or uniformity of points from one class 
...
We can test the spatial interaction of segregation/association or uniformity
...
We can test the spatial pattern of segregation/association or uniformity based on domination of PE-PCD using the function
`TSDomPEBin1D` which is an object of class "`htest`" and performs the same hypothesis test as in `TSArcDensPE1D`.
...
the end interval corrected version of the arc-based test of spatial interaction.
...
The functions for CS-PCD have similar arguments as the `NumArcsPE1D` 
...
### Testing Spatial Interaction or Uniformity with the CS-PCDs
We can test the spatial interaction between two classes/species or uniformity of points from one class 
...
**The Use of Arc Density of CS-PCDs for Testing Spatial Interaction or Uniformity** 
...
and returns the same output as the `TSArcDensPE1D` function.
...
For example, `TSArcDensCS1D(Xp,Yp,int,tau,c,end.int.cor = TRUE)` would yield the end interval corrected version of the arc-based test of spatial interaction.
