...
and the proximity region for a class $\X$ point get larger as the distance of this class $\X$ point
from class $\Y$ points increases.
...
Let $\U(T\left(\Y_3 \right))$ be the uniform distribution on $T\left(\Y_3 \right)$.
If $F=\U(\TY)$, a composition of translation,
rotation, reflections, and scaling, denoted $\phi_b(\TY)$,
will take any given triangle $\TY$
to the standard basic triangle $T_b=T((0,0),(1,0),(c_1,c_2))$
with $0 < c_1 \le 1/2$, $c_2>0$,
and $(1-c_1)^2+c_2^2 \le 1$, preserving uniformity.
That is, if $X \sim \U(\TY)$ is transformed in the same manner to,
say $X'$, then we have $X' \sim \U(T_b)$.
In fact this will hold for data from any distribution $F$
up to scale.
Furthermore, $T_b$ can be transformed to the *standard equilateral triangle*
$T_e=T(A,B,C)$ with vertices $A=(0,0)$, $B=(1,0)$, and $C=(1/2,\sqrt{3}/2)$ by a transformation $\phi_e$
and $\phi_e(X') \sim \U(T_e)$.
That is uniform points in any triangle can be mapped to points uniformly distributed in the standard equilateral triangle
by a composition of $\phi_b(\TY)$ and $\phi_e$ (in the form $\phi_e \circ \phi_b(\TY)$).
...
If $M$ is in the interior of $\TY$,
we use line segments from the center $M$ of $\TY$ to the opposite edges
as extensions of the lines joining the vertices and the center $M$
...
See @ceyhan:comp-geo-2010 for more detail on AS proximity regions.
...
The distribution of the domination number and arc density for AS-PCDs do not change for uniform data in a triangle $\TY$
when the data points are transformed to (uniform) points in the standard basic triangle $T_b$ (using $\phi_b(\TY)$)
but not when $\phi_e$ is applied to the uniform data in $T_b$.
So, one can focus on $T_b$ for computations and derivations regarding the AS-PCD invariants for uniform data.
On the other hand, the distribution of the domination number and arc density for PE- and CS-PCDs do not change for uniform data in a triangle $\TY$
when the data points are transformed to (uniform) points in the standard equilateral triangle $T_e$ (using $\phi_e \circ \phi_b(\TY)$).
That is, distribution of these graph invariants are geometry invariant for uniform data in triangles.
So, one can focus on $T_e$ for computations and derivations regarding the PE- and CS-PCD invariants for uniform data.
