...
region to the opposite edge, and is found by the function `cl2edgesCC_VR`
...
region to the opposite edge, and is found by the function `cl2edgesCM_VR`
which is an object of class `Extrema`. 
Its arguments, `call`, `summary` and `plot` are as in `cl2edgesCC_VR`.
...
Its `call`, `summary` and `plot` are as in `cl2edgesCC_VR`.
We comment the `plot(Ext)` out below for brevity in the exposition, 
as the plot is similar to the one in `cl2edgesCM_VR` above.
...
There is also the function `cl2edgesVRbas.tri` which takes arguments `Xp,c1,c2,M`
...
The description and summary of extrema points and their plot can be obtained using the function `cl2CC.VR`
which is an object of class `Extrema` and takes arguments `Xp,tri,ch.all.intri` where

- `Xp,tri` are as in `cl2edgesCC_VR` and
...
Its `call`, `summary` and `plot` are as in `cl2edgesCC_VR`, except in `summary`
...
There is also the function `cl2CC.VRbas.tri` which takes arguments `Xp,c1,c2,ch.all.intri` and
...
The description and summary of extrema points and their plot can be obtained using the function `fr2vertsCC_VR`
which is an object of class `Extrema`. 
Its arguments, `call`, `summary` and `plot` are as in `cl2edgesCC_VR`, 
except in `summary` distances between the  vertices and 
the furthest points in the CC-vertex regions are provided.
We comment the `plot(Ext)` out below for brevity in the exposition, 
as the plot is a special case of the one in `kfr2vertsCC_VR` with $k=1$ below.
...
There is also the function `fr2vertsCC_VRbas.tri` which takes the same arguments as `cl2CC.VRbas.tri` and 
...
The description and summary of extrema points and their plot can be obtained using the function `kfr2vertsCC_VR`.
which is an object of class `Extrema` and takes arguments `Xp,tri,k,ch.all.intri`
where `Xp,tri,ch.all.intri` are as in `fr2vertsCC_VR` and `k` represents the number of furthest points from each vertex. 
Its `call`, `summary` and `plot` are also as in `fr2vertsCC_VR`, 
except in `summary` distances between the vertices and 
...
There is also the function `kfr2vertsCC_VRbas.tri` which takes arguments `Xp,c1,c2,k,ch.all.intri` and
...
which is an object of class `Extrema` with arguments `Xp,ch.all.intri` which are as in `cl2CC.VR`. 
Its `call`, `summary` and `plot` are as in `cl2edgesCC_VR`, 
...
The description and summary of extrema points and their plot can be obtained using the function `fr2edgesCM_ERstd.tri`
...
region to the opposite face, and is found by the function `cl2facesVRtetra`
...
This function is the 3D version of the function `cl2edgesCC_VR`
Its `call`, `summary` and `plot` are as in `cl2edgesCC_VR`, 
