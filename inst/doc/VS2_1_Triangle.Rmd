...
Alternatively, we can use the `plotDeltri` function in `pcds` to obtain the same plot by executing `plotDeltri(Xp,Tr,xlab="x",ylab="y")`.
...
Indicator for the presence of an arc from a (data or $\X$) point to another for AS-PCDs is the function `IndNAStri`.
...
The function `NumArcsAStri` takes `Xp,tri,M="CC"` as its arguments
...
using the `IncMatAStri(Xp,Tr,M)` command.
...
It takes arguments `Xp,tri,M="CC"` (as in `NumArcsAStri`) and other options for the plot function. 
...
It takes arguments `Xp,tri,M="CC"` (as in `ASarcdens.tri`).
...
Indicator for the presence of an arc from a (data or $\X$) point to another for PE-PCDs is the function `IndNPEtri`.
One can use it for points in the data set or for arbitrary points (as if they were in the data set).
It takes the arguments, `p1,p2,tri,r,M=c(1,1,1),rv=NULL` where 

-`p1,p2,tri,rv=NULL` are as in `IndNAStri` and 
...
Arc density can be adjusted (or corrected) for the points outside the triangle by using the option `tri.cor=TRUE` as in the `ASarcdens.tri`.
...
The incidence matrix of the PE-PCD for the one triangle case can be found by the function `IncMatPEtri`, using e.g. the `IncMatPEtri(Xp,Tr,r,M)` command.
...
It takes arguments `Xp,tri,r,M=c(1,1,1)` (as in `NumArcsPEtri`) and other options for the plot function. 
...
It takes arguments `Xp,tri,r,M=c(1,1,1)` (as in `NumArcsPEtri`).
...
Indicator for the presence of an arc from a (data or $\X$) point to another for CS-PCDs is the function `IndNCStri`.
One can use it for points in the data set or for arbitrary points (as if they were in the data set).
It takes the arguments, `p1,p2,tri,t,M,re=NULL` where `p1,p2,tri,M` are as in `IndNPEtri`
...
Arc density can be adjusted (or corrected) for the points outside the triangle by using the option `tri.cor=TRUE` as in the `NumArcsPEtri`.
```{r eval=F }
...
The incidence matrix of the CS-PCD for the one triangle case can be found by `IncMatCStri`, using the `IncMatCStri(Xp,Tr,tau,M)` command.
...
It takes arguments `Xp,tri,t,M=c(1,1,1)` (as in `NumArcsPEtri`) and other options for the plot function. 
...
the first is for all points, the second is for two $\X$ points only (for better visualization).
It takes arguments `Xp,tri,t,M=c(1,1,1)` (as in `NumArcsCStri`) and other options for the plot function. 
...
It takes arguments `Xp,tri,t,M=c(1,1,1)` (as in `NumArcsCStri`).
...
Alternatively, we can use the `plotDeltri` function in `pcds` to obtain the same plot by executing `plotDeltri(Xp,Te,xlab="x",ylab="y",main="Points in Standard Equilateral Triangle")` command.

Indicator for the presence of an arc from a (data or $\X$) point to another for PE-PCDs is the function `IndNPETe`.
One can use it for points in the data set or for arbitrary points (as if they were in the data set).
It takes the arguments, `p1,p2,r,M=c(1,1,1),rv=NULL` as in `IndNPEtri`.
<!-- Notice that the argument `tri` is redundant for the functions specific to the standard equilateral triangle. -->
```{r eval=F }
...
Number of arcs of the PE-PCD can be computed by the function `NumArcsPETe`which
...
using the `IncMatPETe(Xp,r,M)` command.
...
Indicator for the presence of an arc from a (data or $\X$) point to another for CS-PCDs is the function `IndNPETe`.
```{r eval=F}
P1<-as.numeric(runif.tri(1,Te)$g)
P2<-as.numeric(runif.tri(1,Te)$g)
tau=1
...
using e.g. the `IncMatCSTe(Xp,t=1.5,M)` command.
...
The function `circ.cent.tri` takes `tri` as its sole argument for a triangle and 
...
using the below code, 
see also the help page with `? circ.cent.tri` for the code to generate this figure.
...
The function `circ.cent.bas.tri` is a special case of `circ.cent.tri` and takes the argument `c1,c2`
and returns the circumcenter of the standard basic triangle.

The function `cent.nondegPE` takes arguments `tri,r` 
...
type also `? cent.nondegPE` for the code to generate this figure.
...
The function `cent2edges.tri` returns the projections of a point (e.g., a center) $M$ inside a triangle to its edges,
...
type also `? cent2edges.tri` for the code to generate this figure.
...
The function `cent2edges.bas.tri` is a special case of `cent2edges.tri`
taking arguments `c1,c2` instead of `tri`
and returning projections of $M$ to the edges in the standard basic triangle.

The function `cent.ndPE2edges` takes arguments
...
type also `? cent.ndPE2edges` for the code to generate this figure.
...
The function `as.bas.tri` takes the arguments `tri,scaled` where 
...
The function `tri2std.bas.tri` converts a triangle to the standard basic triangle form, 
...
The function `ind.del.tri` takes the arguments
...
Similarly, the function `indices.del.tri` takes the arguments `Xp,Yp,DTmesh` where
`Xp` is the set of data points for which the indices of the Delaunay
triangles they reside is to be determined and `Yp,DTmesh` are as in `ind.del.tri`.
...
using the function `runifMT` in `pcds` package.
...
of $n_y=$ `r ny` $\Y$ points using the below code. Type also `? indices.del.tri`.
...
The function `rv.tri.cent` takes arguments `p,tri,M`
...
they reside in.
Type also `? rv.tri.cent` for the code to generate this figure.
...
The following functions are special cases of `rv.tri.cent`, where 

* `rv.bas.tri.cent` takes `c1,c2` for `tri` and returns the same output for the standard basic triangle,
* `rv.bas.triCM` takes arguments `p,c1,c2` and returns the indices of the points 
with respect to CM-vertex regions in the standard basic triangle,
* `rv.triCM` takes arguments `p,tri` and returns the indices of the points with respect to CM-vertex regions in a triangle,
* `rvTe.cent` takes arguments `p,M` and returns the indices of the points with respect to $M$-vertex regions 
in the standard equilateral triangle,
and
* `rvTeCM` takes argument `p` and returns the indices of the points with respect to CM-vertex regions 
in the standard equilateral triangle.

The function `rel.verts.tri.cent` takes arguments `Xp,tri,M` for the data set, triangle, 
...
This function is the multiple point (or set) version of `rv.tri.cent`.
```{r eval=F}
A<-c(1,1); B<-c(2,0); C<-c(1.5,2);
Tr<-rbind(A,B,C);
...
The resulting figure will be identical to the one in the examples of `rv.tri.cent`
Type `? rel.verts.tri.cent` for the code to generate the corresponding figure.
...
The following functions are special cases of `rel.verts.tri.cent`, where

* `rel.verts.triM` takes the same arguments and returns the same output as 
and is an alternate to `rel.verts.tri.cent` when $M$ is not the circumcenter,
* `rel.verts.tri.nd` takes arguments `Xp,tri,r,cent=1` for the data set, triangle, the expansion parameter,
...
lines combining the vertices to the center $M$ (i.e., projections of M to the edges) as in the function `cent2edges.tri`.
...
The function `rv.triCC(P,Tr)` takes arguments `p,tri` and 
...
type `? rv.triCC` for the code to generate this plot. 
...
<!-- See `example(rv.triCC)` for the code to generate this figure. -->
...
The function `rv.bas.triCC` is a special case of the `rv.triCC` function,
...
This function is the multiple point (or set) version of `rv.triCC`.
```{r eval=F}
A<-c(1,1); B<-c(2,0); C<-c(1.5,2);
Tr<-rbind(A,B,C);
n<-5  #try also n<-10, 50, or 100
...
The function `re.tri.cent` takes arguments `p,tri,M` where 
...
We can illustrate the CC-vertex regions in an annotated plot,
type `? re.tri.cent` for the code to generate this plot. 
<!-- This plot will be as in Figure \@ref(fig:ER2) without the scatter plot of the data points. -->
This plot will be as in the examples of  `rel.edges.tri.cent` without the scatter plot of the data points.
...
<!-- See `example(re.tri.cent)` for the code to generate this figure. -->
...
The following functions are special cases of `re.tri.cent`, where 

* `re.bas.tri.cent` takes the arguments `p,c1,c2,M` and returns the same output for the standard basic triangle,
* `re.bas.triCM` takes the arguments `p,c1,c2` and returns the indices of the points 
...
and
* `re.triCM` takes the arguments `p,tri` and returns the indices of the points with respect to CM-edge regions in a triangle `tri`.

The function `rel.edges.tri.cent` takes arguments `Xp,tri,M` and 
returns the indices of the $M$-edge regions in a triangle `tri` where the points in a give data set `Xp` reside in.
This function is the multiple point (i.e. set of points) version of `re.tri.cent`.
```{r eval=F}
A<-c(1,1); B<-c(2,0); C<-c(1.5,2);
Tr<-rbind(A,B,C);
n<-5  #try also n<-10, 50, or 100
...
Type also `? rel.edges.tri.cent` for the code to generate this figure.
...
The function `rel.edges.triCM` is a special case of `rel.edges.tri.cent`, and uses the center of mass
